...
$query_kelas = "SELECT k.*, COUNT(s.id_siswa) as jumlah_siswa FROM kelas k LEFT JOIN siswa s ON k.id_kelas = s.id_kelas GROUP BY k.id_kelas ORDER BY k.nama_kelas ASC";
$result_kelas = mysqli_query($koneksi, $query_kelas);

// Jika edit, ambil data kelas
$edit_data = null;
if (isset($_GET['edit'])) {
    $id_kelas = mysqli_real_escape_string($koneksi, $_GET['edit']);
    $query_edit = "SELECT * FROM kelas WHERE id_kelas='$id_kelas'";
...
<head>
    ...
    <title>Kelola Kelas - Sistem Penilaian Sekolah</title>
    ...
    <div class="content">
        <div class="page-header">
            <div class="page-title">
                <h2><i class="fas fa-school"></i> Kelola Kelas</h2>
                    ...
                    <tbody>
                        ...
                        if (mysqli_num_rows($result_kelas) > 0) {
                            while ($kelas = mysqli_fetch_assoc($result_kelas)) {
                                ...
                                echo "<a href='kelola_siswa.php?kelas={$kelas['id_kelas']}' class='btn-edit'><i class='fas fa-eye'></i> Lihat Siswa</a>";
                                echo "<a href='?edit={$kelas['id_kelas']}' class='btn-edit' onclick='openModal({$kelas['id_kelas']})'><i class='fas fa-edit'></i> Edit</a>";
                                echo "<a href='?hapus={$kelas['id_kelas']}' class='btn-delete' onclick='return confirm(\"Apakah Anda yakin ingin menghapus kelas ini?\")'><i class='fas fa-trash'></i> Hapus</a>";
                                echo "</td>";
                                echo "</tr>";
                            }
                        } else {
    ...
    <div class="modal-content">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nama_kelas" name="nama_kelas" required placeholder="Contoh: X-1, XI-IPA-1">
            </div>
...
function openModal(id = null) {
    ...
    if (id) {
        ...
        document.getElementById('nama_kelas').value = '<?php echo $edit_data['nama_kelas']; ?>';
        <?php endif; ?>
    } else {
